...
export default function PlantpawPrivacy() {
  return (
    <PawPage app={app} kind="privacy">
      <Section title="한 줄 요약">
        <p>
          잎새봄은 계정과 로그인이 없는 식물 이력장입니다. 식물 기록은 이 기기에만 저장되고, 이 버전은 사용 분석
          데이터를 전송하지 않습니다.
        </p>
      </Section>
      <Section title="기기에 저장되는 정보">
        <p>
          화분 이름, 점검 간격, 물줌·살펴봄·미룸 기록과 관찰 메모, 그룹 구성, 구매 상태가 이 기기의 앱 저장 공간에 로컬
          JSON 파일로 저장됩니다. 개발자 서버나 클라우드 동기화는 없습니다.
        </p>
        <p>
      ...
      <Section title="CSV 내보내기">
        <p>
          유료 확장의 CSV 내보내기는 사용자가 기간을 골라 직접 실행할 때만 만들어지고, 사용자가 고른 곳으로만
          전달됩니다. 내보낸 파일은 암호화되지 않은 일반 파일이므로 보관과 공유는 사용자가 관리합니다. 개발자는 내보낸
          파일을 받지 않습니다.
        </p>
      </Section>
